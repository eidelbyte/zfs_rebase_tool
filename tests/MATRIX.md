...
| ZX253 | --continue -i on a rebase at applying2 is refused with exit 2 before anything is taken over, the way --no-merge is refused there: the gate the child opens at is behind it | planned: box, box/run-resolution.sh case 12, beside the --no-merge refusal it already makes from that phase; continue_from reads the phase off a real record, so no unit test can reach the dispatch |
...
| ZX254 | -o pointing at a directory that exists is refused by the guard, before the run directory and the pre-apply snapshot, rather than by the rename at the birth manifest with exit 3; a symbolic link to a directory is not refused, since the rename replaces the link | covered: check_run.c |
