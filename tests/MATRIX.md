...
| ZX260 | the gate snapshot is taken at the hand-off into applying2, after the document is found complete and before the phase moves and before a byte of applying2 is written: <result>@zfs_rebase-<tag>-gate in both forms, the tool's own and named like the tool's other one, and a hand-off that finds one already recorded replaces it, so one rebase has one gate snapshot at a time | planned: box, box/run-resolution.sh, a case beside case_drift that reads zfs list -t snapshot on the result at the applying2 pause gate |
| ZX261 | it is in the record, as zfs_rebase:gatesnap, written after the snapshot exists and taken off with the other three properties at done and at --abort, so nothing outlives the rebase that names it | planned: box, the same case and box/run-kills.sh's property assertions at the applying2 gate |
...
| ZX263 | --abort on a rebase at applying2 rolls the result back to the gate snapshot and stops there: the hand merges made at the gate are in the tree, the phase is back at conflicts, the record, the holds, the two documents and the run directory are all still there, the result is still at the private mount, the exit is 0, and a second --abort now finds a rebase at conflicts and takes the whole thing down as it always did | planned: box, box/run-kills.sh: a kill at applying2, then --abort, then the object screen 2's w merged at the gate is still there with the merged bytes, then --abort again |
| ZX264 | and everything that destroys or rolls back the result takes the gate snapshot away first, so that none of them meets an EEXIST it did not have before: --abort's full path in both forms (lzc_destroy refuses a clone that has a snapshot, and a rollback to the pre-apply snapshot refuses while a later one stands), --restart in both forms, and --abort before the gate, which is unchanged because there is no gate snapshot to find | planned: box, box/run-kills.sh's settle cases and box/run-resolution.sh case_restart, each run once from applying2 |
...
| ZX256 | the fresh run checks its own stop flag between the -i child's return and the hand-off into the gate function, as it does at every other boundary: a SIGHUP or a SIGINT that arrived while the editor had the terminal stops the run at the conflicts gate, with the gate standing and the kept-result hint printed, instead of being noticed first inside applying2 (question 8c of open-questions-2026-09-15.md, recommended and agreed) | planned: box, box/run-kills.sh beside its kill cases: a fresh run with -i whose editor lingers, a SIGHUP to the tool while it lingers, the editor then exiting 0 -- the run must stop at conflicts with the record and the holds standing, and a later --continue must take it on. No unit test can reach it: zr_run wants a pool and root, and check_run.c has no fresh-run harness |
| ZX255 | the resume path lets its three walks and its oracle go before the -i child is forked and makes them again after the wait, as the fresh run does (release_trees before its own child): an editor that changes a conflicted object at the result's private mount -- a chmod, an attribute, a name removed -- while it answers the document reaches done, the change showing as drift at the done gate, where a walk carried across the child put that change into applying2's second pass instead of its first and gave the "did not make the document true" internal failure that blames the tool for a person's edit; a removed name made the oracle's open fail outright. This is also the assumption under G5's recovery story, which it settles (L1 of the code review of 2026-09-11) | planned: box, box/run-resolution.sh case 12, a case beside case_i_drift with a mode of its own in the case-12 editor -- keep's answering plus a chmod of a conflicted object at $ZR_ED_TREE before it writes. No unit test can reach it: resume_interactive and stage_conflicts are static over a struct resume private to run.c, reached only from continue_from with a phase read off a real dataset property, and the window itself is between a fork and a wait around a real child at a real private mount (the same reason as ZX253) |
