...
| ZP163 | a metadata pick, q, w (or m, f, w with nothing else unpicked): the result's mode is from's, because f picked from's mode in the metadata three-way and w wrote the resolved attributes | covered: check_picker.c on a pty, m, f, w, q; the result's mode is 0640 |
