...
| ZX262 | released at the done gate: destroyed in the same step as the tool's own from snapshot where the final check found no drift, and kept where it found drift, with one line naming it and saying how to roll back to it and how to destroy it -- the author's "release it when the verification pass reports that the actions didn't corrupt" | planned: box, box/run-fixture.sh's done in both forms for the clean half, box/run-strays.sh's drifted done for the other |
