...
| ZX270 | a verb that opens an -i child writes <rundir>/session first: the two pids and the two start times, the verb, what -i named and when, through the sibling-and-rename primitive every other document goes through, and unlinks it when the child is reaped whatever the child exited with (tracker issue concurrent-continue, ruled 2026-09-15: "Yes to both") | covered: check_run.c for the write, the read back and the round trip; box for the file's life around a real child, box/run-resolution.sh case 12 |
| ZX271 | a verb arriving on a rebase whose session file names a process that is still alive is refused before it takes the result over and before it reads a tree, with one line naming the pid, the editor and when it was opened, and exit 2; the child is tested first, since it is the one with the tree in its hands | covered: check_run.c for the liveness rule over a file the test writes; planned box for the refusal itself, box/run-resolution.sh, a case beside case_i_killed: one -i run left in the background at the gate and a second zfs_rebase --continue on the same rebase from the same script |
| ZX272 | and a plain --continue is refused by the same check and for the worse reason -- it goes on into applying2, which writes over the tree the other person's editor still has open -- as are --restart and --abort; --verify is not refused, since it reads, writes nothing and takes nothing over | planned: box, the same case, which runs all four verbs against the live session and then --verify, which must report as usual |
| ZX273 | a session file whose processes are gone is stale: the next verb reads it, finds neither pid alive, says nothing and goes on, and the next child writes over it. That is what a SIGKILL of the tool leaves, and it is why liveness is asked of the system and not of the file | covered: check_run.c, which writes a file naming a process it has reaped; planned box, box/run-resolution.sh case_i_killed, whose kill already leaves one |
| ZX274 | the file is gone with the run directory: done and --abort both remove it, since every path that removes that directory goes through rmdir_run, and a directory still holding it would not rmdir | covered: check_run.c for the path it composes; planned box, the gate assertions of box/run-kills.sh, which already assert what the run directory holds |
| ZX275 | a reused pid is told from the pid that was recorded by the process's start time, read from the system at both ends; where the system will not say the start time is 0 and the pid alone decides, which is the one place this check can read a stranger as the process it is looking for and refuse a verb that should have gone on | covered: check_run.c over the liveness rule with a start time that does not match; the fallback is FreeBSD's and the mac's alike, since both answer KERN_PROC_PID |
