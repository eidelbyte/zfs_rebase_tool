...
| ZP166 | the ACL row shows the entries as acl_to_text lines with numeric ids, one line per entry, with the cell as tall as the longest of its three columns | planned: box, by hand, hand-sessions.md; the mac's extended ACL text is verified by the drawing code on macOS with a real ACE |
...
| ZP191 | an ACL cell draws each entry from its first character: FreeBSD's acl_to_text_np pads entries on the left so a whole ACL's colons line up, which put them flush right in a cell of their own. The value columns take a quarter of the width left after ATTR, with no upper cap, so an entry is cut as late as the screen allows (the author, 2026-09-23: "the acls are right aligned") | planned: box, by eye; the padding is FreeBSD's text form alone, and the mac's ACL text has none |
