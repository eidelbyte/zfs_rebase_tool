...
| ZM95 | a decoded NUL in a name is refused by both parsers, as "/", "." and ".." are: a line named a\000zzz would describe, open and truncate /a, since the apply hands the decoded bytes to openat as a C string (M3 of the code review of 2026-09-11) | covered: check_manifest.c |
| ZM96 | and a decoded NUL in the argument of an ln, a cp or a write, which is the same bytes reaching the same calls by the other road | covered: check_manifest.c |
...
| ZX58 | the report takes no hold on the inputs it reads: an open rebase's inputs are held by the record's own three holds and a settled check reads what is there. It used to take a temporary one per input, and the kernel answers every release of a hold, temporary or not, by unmounting the snapshot's .zfs view (dsl_userhold.c, zfs_unmount_snap), which on FreeBSD is a forced unmount (zfs_ctldir.c, MS_FORCE) that dooms the descriptors of every other process reading that snapshot -- the --continue paused inside applying2 beside a --verify came back to "open of the from file: Not a directory" on the root it held (the box, 2026-09-16, tools/probe-snapfd.c) | covered: box/run-resolution.sh, case_twoverbs: the paused --continue reaches done after the --verify beside it; and zfs holds shows no zrv- tag after a report |
...
| ZX182 | the cross-check, both halves: the record must name the file given, and the manifest the record names must name this result back, and either mismatch is exit 2 with both sides in the message | covered: read_manifest and check_given in src/run.c; box, box/run-fixture.sh 3a, which gives a copy of this rebase's own manifest and a manifest whose header names another run, and box/run-strays.sh case 9 for the swapped -o path, where the swap is made by the harness with a copy: -o will not write over a path that exists (ruled 2026-09-15), so the second run is given a path of its own and what is under test is the swapped state and not how it was reached |
...
| ZX245 | the composed name is the clone's, and not only the header's: the clone is created under it, mounted at the run's private mount under it, found under it by --continue and --abort, and destroyed under it at done and at --abort (5fc39a3) | covered: box/run-resolution.sh, case_barename, in the clone form: a fresh run with --result as a bare word, the composed clone created, mounted at the private mount, found under the short name by --verify and --continue, and settled under it |
...
| ZX251 | and the same over a real rebase: one left at applying1, a conflict line deleted by hand with the two counts corrected, then --continue and --continue -i; the line is back in the file with "-", the run stops at the gate with the unanswered count, and nothing reaches applying2 without it | covered: box/run-resolution.sh, case_putback1, run both ways (plain and interactive): a kill at the applying1 pause, a conflict line dropped by hand, then --continue and --continue -i -- the line back reading "-", the counts back, the run stopped at the gate with the count, and the -i child handed a document that already has it |
| ZX252 | the done gate's order in the clone form: the final check, the walks closed, the clone unmounted, and readonly written on only after that; a done whose unmount refuses leaves the rebase open on a clone that is still writable, and the next --continue can write in it | covered: box/run-kills.sh, settle_busy_done, the clone form: after the refused settle the result still reads readonly=off, so the open rebase is writable and the next --continue can finish it |
| ZX253 | --continue -i on a rebase at applying2 is accepted and idle: -i takes effect at the conflicts gate and nowhere else, so past the gate the run keeps applying exactly as a --continue without it would, reaching done with the same exit and no word about the flag (ruled 2026-09-15, reversing L4 of the code review of 2026-09-11: "-i is allowed but idempotent"). --no-merge is still refused there, which is the one flag that is | covered: box/run-resolution.sh, case_i_applying2, beside case_killchoice's --no-merge refusal from the same state: --continue -i from applying2 exits 0 and reaches done, and ed_ran 0 says no child opened |
| ZX270 | every verb that takes the result over writes zfs_rebase:active as soon as it has read the record and before the take-over: the fresh run at its birth with the rest of the record and before applying1, and --continue, --restart and --abort in resume_found or their own path. The value is the verb's own pid and the moment that process started, "<pid> <start>" (tracker issue concurrent-continue, re-ruled 2026-09-16: "i think i'd rather just use another userprop ... and do this for every run invariantly") | covered: by hand on the box, 2026-09-23, rewritten hand session 1 of box-trip-2026-09-16.md on tool fdab53d; covered: check_run.c for the value's form; planned box, box/run-resolution.sh, reading the property at the gates a pause holds |
| ZX271 | a verb that finds it set on a process that is still alive is refused before it takes the result over and before it reads a tree, with one line naming that process and the value that says so, and exit 2 | covered: check_run.c for the liveness rule over a value the test composes; planned box, box/run-resolution.sh, a case beside case_i_killed: one -i run left in the background at the gate, and a second zfs_rebase --continue on the same rebase from the same script |
| ZX272 | and two plain verbs are the same refusal and the point of the re-ruling: a --continue arriving while another --continue is applying, and an --abort arriving while a --continue is applying, each meet a live process and are refused, where before nothing held a rebase at all and take_over adopted the private mount the other verb was working in. --verify is refused by none of them and sets nothing | covered: box/run-resolution.sh, case_twoverbs: a --continue held at the applying2 pause, then --continue, --restart and --abort each refused with exit 2 naming that pid, --verify not refused, and the property gone with the record at done |
| ZX273 | a value whose process is gone is stale: the next verb reads it, finds the process dead, says nothing and writes its own over it. That is what a SIGKILL of a verb leaves, and it is why liveness is asked of the system and not of the property | covered: by hand on the box, 2026-09-23, rewritten hand session 1 of box-trip-2026-09-16.md on tool fdab53d; covered: check_run.c, which composes a value naming a process it has forked and reaped; planned box, box/run-resolution.sh case_i_killed, whose kill already leaves one |
| ZX274 | it is cleared when the verb ends whichever way it ended -- clean, refused, interrupted, or stopped at a gate with the rebase standing -- and goes with the rest of the record at done and at --abort, so a settled or aborted rebase carries none of it | covered: by construction, resume_close and the fresh run's teardown and zr_record_props; planned box, box/run-kills.sh's property assertions, which already read the record at every gate |
| ZX275 | a pid the system has since given to somebody else is told from the verb that set the property by the process's start time, read at both ends from KERN_PROC_PID; where a system will not say, the start time is 0 and the pid alone decides, which is the one place this can read a stranger as the verb it is looking for and refuse one that should have gone on | covered: check_run.c, which moves the start time by one microsecond and watches the same live pid stop being the verb |
| ZX260 | the gate snapshot is taken at the hand-off into applying2, after the document is found complete and before the phase moves and before a byte of applying2 is written: <result>@zfs_rebase-<tag>-gate in both forms, the tool's own and named like the tool's other one, and a hand-off that finds one already recorded replaces it, so one rebase has one gate snapshot at a time | covered: box/run-resolution.sh, case_gatesnap: the tool held at the first choice of applying2, exactly one <result>@zfs_rebase-zr-*-gate on the result there, and none after a clean done |
| ZX261 | it is in the record, as zfs_rebase:gatesnap, written after the snapshot exists and taken off with the other three properties at done and at --abort, so nothing outlives the rebase that names it | covered: box/run-resolution.sh, case_gatesnap: zfs_rebase:gatesnap at that pause names that snapshot, and the whole record is off the result at done |
| ZX262 | released at the done gate: destroyed in the same step as the tool's own from snapshot where the final check found no drift, and kept where it found drift, with one line naming it and saying how to roll back to it and how to destroy it -- the author's "release it when the verification pass reports that the actions didn't corrupt" | covered: box/run-strays.sh, the stray before the done gate, both forms: the drifted done keeps exactly one gate snapshot on the result and names it with the rollback and the destroy; the clean done's release is every other done of the box scripts, whose resets destroy the result without -r |
| ZX263 | --abort on a rebase at applying2 rolls the result back to the gate snapshot and stops there: the hand merges made at the gate are in the tree, the phase is back at conflicts, the record, the holds, the two documents and the run directory are all still there, the result is still at the private mount, the exit is 0, and a second --abort now finds a rebase at conflicts and takes the whole thing down as it always did | covered: box/run-resolution.sh, case_abort_applying2: a mark written at the gate, a kill inside applying2, then --abort -- exit 0, the line naming the gate, the phase back at conflicts, the mark still in the tree, three holds, both documents, the run directory and the snapshot all there; end_case's own --abort is the second one and takes the rebase away |
| ZX264 | and everything that destroys or rolls back the result takes the gate snapshot away first, so that none of them meets an EEXIST it did not have before: --abort's full path in both forms (lzc_destroy refuses a clone that has a snapshot, and a rollback to the pre-apply snapshot refuses while a later one stands), --restart in both forms, and --abort before the gate, which is unchanged because there is no gate snapshot to find | covered: box/run-resolution.sh, case_restart_applying2, both forms: a --restart from applying2 exits 0 rather than meeting the EEXIST a standing gate snapshot would give the destroy or the rollback, and leaves no gate snapshot behind; --abort's full path is the same drop, made by case_abort_applying2's second abort and by every end_case that follows one |
| ZX265 | the "second pass did not make the document true" refusal names the gate snapshot and the --abort that returns to the gate, so the one message a person meets when applying2 malfunctions says what to do about it | covered: check_run.c for the composed name the message carries; box for the message itself, box/run-resolution.sh, which cannot provoke the refusal without a fault injected and so reads the wording from the source |
| ZX266 | the gate snapshot's name is a function of the result and the tag: <result>@zfs_rebase-<tag>-gate, the tool-made prefix with a suffix of its own so that it is never the name the tool takes for a side given as a dataset, and a name that will not fit is refused rather than cut | covered: check_run.c |
| ZX267 | the --posix form takes no snapshot at all -- it compares three directories and owns no dataset -- so there is no gate snapshot in it, no record to carry one and nothing for --abort to land on; --abort in that form is what it always was | covered: by construction, zr_run's posix path never reaches stage2 and takes no snapshot; the fixtures pass exercises it on every run |
| ZX256 | the fresh run checks its own stop flag between the -i child's return and the hand-off into the gate function, as it does at every other boundary: a SIGHUP or a SIGINT that arrived while the editor had the terminal stops the run at the conflicts gate, with the gate standing and the kept-result hint printed, instead of being noticed first inside applying2 (question 8c of open-questions-2026-09-15.md, recommended and agreed) | covered: box/run-resolution.sh, case_i_hup: a SIGHUP to the tool while the editor lingers, the run stopping at the conflicts gate with exit 1 and the interrupted line, no gate snapshot on the result, and a --continue that then reaches done |
| ZX255 | the resume path lets its three walks and its oracle go before the -i child is forked and makes them again after the wait, as the fresh run does (release_trees before its own child): an editor that changes a conflicted object at the result's private mount -- a chmod, an attribute, a name removed -- while it answers the document reaches done, the change showing as drift at the done gate, where a walk carried across the child put that change into applying2's second pass instead of its first and gave the "did not make the document true" internal failure that blames the tool for a person's edit; a removed name made the oracle's open fail outright. This is also the assumption under G5's recovery story, which it settles (L1 of the code review of 2026-09-11) | covered: box/run-resolution.sh, case_i_chmod: the case-12 editor in its chmodtree mode answers the document onto -- so that applying2 acts on every conflicted name -- and chmods one of those names at the private mount while it has the tree. The run must exit 0 with no "did not make the document true" line and the name onto's again at done: with a walk carried across the child the first pass misses the chmod on stale metadata and the second makes it, which is that refusal. The change shows as drift at no gate, because the stage now sees it and mends it, which is the point |
| ZX254 | -o pointing at anything that already exists is refused by the guard -- a regular file, a directory, a symbolic link, a dangling symbolic link -- before the run directory and the pre-apply snapshot, with one line saying the path exists and that -o names a file to create; the containing directory must still be there, be a directory and be writable, and a path with nothing at it passes (ruled 2026-09-15, widening L5 of the code review of 2026-09-11: "-o to existing should fail"). The question is asked with lstat, so a dangling link is something that exists, and only on the fresh run, which is the one caller | covered: check_run.c |
...
| ZY120 | the sorted index of the manifest's marks answers "does the manifest mark this exact name" exactly as the scan it replaces did, over a manifest of many marks: every marked name, every unmarked one, a name that is a prefix of a mark and a mark that is a prefix of another, and a manifest with no mark at all (L7 of the code review of 2026-09-11) | covered: check_verify.c |
...
| ZI19 | SIGINT while the tool waits is decisive: the parent kills the child with SIGTERM, reaps it and reports the launch failed whatever the child's status, so a child that ignores SIGINT and would have exited 0 does not carry the run past the gate; the child is gone, the three dispositions are put back, and the tool is still there to say so (ruled 2026-09-15: "Ctrl+C should probably kill the editor in any phase") | covered: check_run.c |
...
| ZI36 | on the resume path the built-in child gets base's directory as the fresh run's does, found from the record like a side's; add/add only where the header has no base | covered: by hand on the box, hand session 1 of box-trip-2026-09-16.md (hand-sessions.md, 2026-09-17): the resume path's screen 2 read E/E |
| ZI37 | the child opens on a result that is writable already, on both paths, and no flip is made for it or after it; the picker's merge write lands | covered: by hand on the box, hand session 1 of box-trip-2026-09-16.md (hand-sessions.md, 2026-09-17): w on screen 2 wrote, on both paths |
...
| ZI41 | SIGQUIT while the tool waits is the same as SIGINT and for the same reason -- Ctrl-\ is the terminal's other way of saying stop and an editor may swallow it too -- so a child that ignores it and exits 0 still leaves the launch failed | covered: check_run.c |
| ZI42 | the termios belt falls back to /dev/tty where standard input is not a terminal: one open before the fork, the same save and restore around the child, the descriptor closed after, so a run whose stdin is redirected still hands the terminal back as it found it | covered: check_run.c, in a fork of the test's own that takes a pty as its controlling terminal (setsid then the slave without O_NOCTTY) and puts /dev/null on its standard input; skipped with a line where no pty or no controlling terminal can be had |
| ZI43 | the tcsetattr of the belt runs with SIGTTOU ignored, so a child that took the foreground and died cannot stop the tool on the way out, and the tool's own SIGTTOU disposition is what it was afterwards | covered: check_run.c, beside ZI21 on the same pty |
...
| ZP55 | every line comes back with the choice the keys gave it, for a sequence that answers the rows back to front; the ORDER is the library writer's walk order and not the order the file was read in, and a directory line that scopes nothing is a scope to the parser and is not written (the review of 2026-09-11, M8, which measured both; the writer is shared with every gate write, so the picker is in step with the tool) | covered: check_picker.c |
...
| ZP61 | a crash between the rename and the exit: the written document stands, no .tmp beside it, and the next --continue reads it | covered: by hand on the box, 2026-09-23, rewritten hand session 1 of box-trip-2026-09-16.md on tool fdab53d; before that planned: box, by hand, in the worklog of picker-list |
...
| ZP95 | add/add with no base: the two-way compare of from against onto, in that order, with a pick and under the same write rule. What it claims: every pick of from gives from's lines, every pick of onto gives onto's, the lines the two sides share appear once and are stable, and every base range is empty. What it does NOT claim: the swap property (ZP107). The sequence is finer than the walk over an empty base object would give and is not the mirror of itself under a swap, by the author's ruling of 2026-09-15 -- "the two-way shortcut is fine, since the user specifies from and onto, we'll just always do diff(from, onto)" -- so from and onto are the person's names and not two interchangeable sides (v4-merge3.md section 5, and the review of 2026-09-11, G3) | covered: check_merge.c, check_add_add over the four properties it does claim, with check_swap deliberately not applied to it and the reason in the comment; check_picker.c for the row that opens with no base object, takes a pick and writes |
...
| ZP100 | a text object of tens of megabytes: the merge finishes and the panes scroll, libdiff's divide-and-conquer path taken | covered: by hand on the box, hand session 1 of box-trip-2026-09-16.md (hand-sessions.md, 2026-09-17): a 40 MB /A on all three trees, the start and the gate near-instant by the author's wristwatch; the merge opened, n, p and c usable, w near-instant, all by the author's word |
| ZP101 | the write is in place, so the object's mode, owner, times and extended attributes stand | covered: by hand on the box, hand session 1 of box-trip-2026-09-16.md (hand-sessions.md, 2026-09-17): inode 3 stood, mode 0640 stood, user.zrmark stood with its value, size and mtime moved; check_picker.c holds the inode and the mode across a write on the mac |
| ZP102 | the two panes scroll together: a filler line in one is a real line in the other, and the chunk under the cursor is the same in both | covered: by hand on the box, hand session 1 of box-trip-2026-09-16.md (hand-sessions.md, 2026-09-17): the panes aligned row for row, n and p moved them together, c folded the agreed stretches |
| ZP103 | a text conflict merged on the box, the rebase taken to done, and the final check reading that name at keep | covered: by hand on the box, hand session 1 of box-trip-2026-09-16.md (hand-sessions.md, 2026-09-17): f, w, w, exit 0, done, the body merged as diff3 merges it |
...
| ZP107 | swapping from and onto swaps the conflict halves and nothing else, over a merge that has a base | covered: check_merge.c over tests/battery/, every case of which has a base object; the add/add form is outside this cell by the ruling of 2026-09-15 and says so in ZP95 |
...
| ZP118 | screen 2's cell layout: the gutter character, a space, the cursor's cell, a space, the line number right-aligned in the view's own number width, two spaces, the text; and the gutter reads blank for a stable stretch, + for a stretch that is one side's own, and f or o for a conflict, saying which key answered it (935705e, e2b678f) | planned: check_picker.c on a pty for the gutter characters over a merge with one hunk taken each way, and box, by hand, for the layout itself; the number's width is ZP119's |
| ZP119 | the result pane numbers the merged file and not a side: the numbers run 1..n over the lines the result would hold, so a line taken from onto carries the number it will have and not onto's (e2b678f); the column is as wide as the largest number the view can draw -- the longest of the three files, or the result the answers make -- so a file above 9999 lines draws its numbers whole; and the hunk's own answer is counted while b shows base in its place, so the numbers after the cursor's hunk do not fall behind (the review of 2026-09-11, S6) | covered: check_picker.c on a pty -- a conflict near line 50000 of a 100000-line file, whose text holds no digits, so a five-digit number on the screen is the assertion; and the small merge fixture read after b, where the base rows are drawn and not one digit is rewritten, because no number moved. The two sides of different lengths, which tells a result number from a side's, is still planned there |
...
| ZP123 | SIGABRT, SIGILL and SIGFPE leave the terminal as the six handled signals do: the termios back, the alternate screen left where the picker entered it, and the signal re-raised with its default disposition; and the picker's own sources raise none of them themselves -- no assert(3) is left in them, the walk's step check being the queued refusal that already stood beside it | covered: check_picker.c on a pty, the three added to the sweep of ZP65 to ZP67, with the enter and leave sequences read out of the recorded stream; the source half is a grep of src/plugins/picker/*.[ch], recorded in the worklog of review2-picker, the carried libdiff excepted (never edited; -DNDEBUG in the shipped flags is the build's half). __stack_chk_fail and a libc heap abort raise SIGABRT too, so the cell is the handler's and not the assert's; the box confirms SIGABRT on a real terminal, one hand session in the worklog |
...
| ZP125 | a second signal arriving while the first restore is still running: the terminal is still put back, because the one call that cannot block runs first and unguarded; and the second signal waits in the handler's mask for the first teardown, so endwin runs to its end -- the alternate screen left, the cursor shown -- and the picker dies of the first signal | covered: check_picker.c on a pty whose master is left unread until the picker blocks inside curses, SIGINT then SIGTERM, the settings read both mid-teardown and at the end, the drained output holding the alternate-screen exit and cnorm, the status SIGINT. The old order fails it with a raw terminal and a status of signal 15, which is the verifier's own result; an empty handler mask fails it with the screen never left and a status of signal 15, which is what the box showed on a real Ctrl-C (hand session 2, 2026-09-18) |
| ZP126 | an escape sequence the terminal sends that terminfo does not describe: the bytes after the escape are not read as a bare Esc, so nothing of what was answered is discarded by a key nobody pressed | covered: check_picker.c on a pty -- a focus report and a cursor key in the other keypad mode on the list, then the answers and w, which exits 0 with the document written; the same sequence inside a merge, which comes out keep; and a lone escape, which still leaves with nothing written |
| ZP127 | a scoping directory row -- a directory line with no action of its own in the manifest (zk_isdir and not ZR_PK_L_CONFLICT) -- shows "/" in the CHOICE column, not "-": the glyph says directory and cannot be read as unanswered (ruling 48, superseding ruling 12) | covered: check_picker.c, a hand-added directory conflict line whose name the manifest does not mark, opened beside a self-conflicted directory that still shows its own answer |
| ZP128 | a key pressed on a scoping directory row does nothing but say so in the key bar: f, o, k, - and Enter are all refused with one line naming the row, and no choice is set (ruling 48, superseding the apply-down of ruling 12; a recursive take under capital F, O and K is future work) | covered: check_picker.c, every choice key and Enter on a scoping directory row, each refused with a message and the choice unchanged |
| ZP129 | the unanswered count never includes a scoping directory row, even when the row's document line reads ZR_CH_NONE: the person is not asked to answer a line that has no action behind it (ruling 48) | covered: check_picker.c, a scoping directory that reads "-" on disk and is not counted in zc_unanswered |
| ZP130 | the merged bytes go into a regular file or nowhere: a symbolic link at the name in the result tree is refused and never followed, a fifo is refused and never blocks the open, the row does not move and nothing outside the tree is touched (M2 with G6, the field M5 found unread) | covered: check_picker.c, a link out of the tree and a fifo, both at a name whose three sides are text |
| ZP131 | a tree the picker cannot read -- absent, not a directory, or unsearchable -- is one queued line naming the tree, its path and what the system said, at open and before a row is built, rather than every name there reading as absent in silence (M6) | covered: check_picker.c, the base tree at mode 0000, skipped when the tests run as root |
| ZP132 | a file of more than ZR_M3_MAXLINES lines is refused before libdiff is asked for anything, with the count and the ceiling in the line (G2, which is what keeps the carried patience diff's unchecked allocations of G1 out of reach) | covered: check_merge.c, a file of ZR_M3_MAXLINES + 1 lines built in memory, skipped when it cannot be allocated |
| ZP133 | a merge write that fails after the open has truncated: one queued line naming the object and the reason, and the row back to unanswered whatever it read before, so a row that already read keep cannot ship a short object in silence (G5) | covered: check_picker.c under RLIMIT_FSIZE with SIGXFSZ ignored |
| ZP134 | a manifest of many groups: every conflict row finds its own record, and the header counts the distinct groups, with no scan of the records per row and no dedupe in a double loop (M4) | covered: check_picker.c, a document of many one-name groups, the record checked on every row |
| ZP135 | the inside-conflict hint is taken once for the chunk the cursor is on and kept across keys that do not move it, and dropped when it does move, the marks drawn being the same either way (G4) | deferred: the hint changes no byte of any answer and only the color of a line, which this harness does not decode, so nothing it could assert would fail; the behaviour rides on the screen-2 pty sessions (ZP40, ZP82, ZP85, ZP88), which press keys that do and do not move the cursor and hold the drawing and the written bytes, and the cost before and after is in the worklog of review2-picker. Unblocking work: a terminal emulator in the harness that decodes attributes, which would let the dim marks be read |
| ZP136 | after a merge write the message says the merged bytes are written, that the name reads keep and that the resolution is saved, since screen 2's w now writes both (the author, 2026-09-15, on M9); where the document could not be written the same message says so and names the reason instead | covered: check_picker.c |
| ZP137 | screen 2's w writes the merged bytes into the result's object and then the document, in that order and through the one writer the list's s and w use, so the tree and the resolution never disagree on disk: after it the document on disk reads keep for that name. A failed object write is ZP133's and the document is not touched; a failed document write leaves the row keep in memory with one line naming the object and the reason, the object written, the document as it was, and the next s or w retrying it (the author, 2026-09-15, on M9) | covered: check_picker.c on a pty, a session that opens the merge, answers the hunk, writes with w and leaves with q, the resolution read back from disk afterwards; and check_picker.c over the model for the failure, with the directory holding the document made unwritable, skipped when the tests run as root |
| ZP138 | q on the list with answers no write has saved asks once in the key bar: s saves and leaves with 1 and says nothing, d leaves with 2 and one line naming how many answers were not saved, printed after the terminal is the person's again, any other key cancels and the list stays; with nothing unsaved q leaves at once and says nothing (the author, 2026-09-18, hand session 6: "I wish we would have a warning, unwritten changes pop-up (for both screen 1 and screen 2)", over the no-question half of M9; zr_pk_dirty is the count's source) | covered: check_picker.c on a pty for d, s, cancel and the bare q, the line asserted to lie past the last escape byte the way ZP68 does it, and over the model for the count itself |
| ZP139 | a merged object with more than one name in the result tree: w on one of them sets every row whose result object is that same file -- same device and inode, recorded at open and confirmed by a fresh stat at the write -- to keep, and the one document write carries all of them, since a pool is one file and every name it has (v4-manifest.md section 2, and the author's ruling of 2026-09-15 on the review's question 14). A row of the same group whose result object is a different file is not touched, and neither is one of another group; the message says how many names were set | covered: check_picker.c over the model, with a real hard link made in the test's result tree and a third name of the same group that is its own file; and check_picker.c on a pty, the document read back from disk with both names at keep |
| ZP140 | the detail line names the result object's link count where it has more than one, so the person can see before pressing w that the name they are on is not the only one that will change (the review's question 14, recommended and ruled) | covered: check_picker.c on a pty, the detail pane read over a row whose result object has two names, and over one that has a single name, which says nothing about links |
| ZP141 | the C merge held against an outside implementation on inputs the battery does not have: a generated corpus, deterministic from a seed, of a base and two sides derived by one to four edits of every kind (a replacement, an insertion, a deletion and a block moved), each case given a shape before it is built -- clean, mixed with an identical edit on both sides, or conflicted -- and run through both our zr_m3_open and an outside merger. The authority is chosen at run time and printed: FreeBSD's diff3 where it is 20260213 or later (2cfca8e710f2 fixed a merge mode that lost lines) or GNU diffutils' diff3, which FreeBSD 15.1-RELEASE ships in base and which the battery was recorded against, otherwise git merge-file -p --diff3 where git is installed, otherwise none and the classification alone. Every case falls in one of three boxes and nothing else: both clean, and then the merged bytes are compared; ours holding a both-same chunk where diff3 brackets it, the one known translation, which git does not need; or both conflicting, where the shapes are left uncompared. A case outside the three fails the run, naming the seed, the case, the three inputs under the build directory and what each side said. Where both merged cleanly and the bytes differ, the case is counted as aligned differently and printed rather than failed, and ours is held to the properties of v4-merge3.md section 7 instead -- two correct mergers may align a repeated stretch differently and both be right. The add/add fifth has no base for an outside merger to take and is held to the properties of section 7 (ZP95) | covered: check_merge.c --oracle, run by make oracle and by make check at seed 20260915 over 60 cases of 3 to 400 lines (16 clean, 16 mixed, 16 conflicted, 12 with no base; about a second, most of it the fork of the outside merger). The box runs make oracle ORACLE_ARGS="20260915 240 4", which is 240 cases and four more of 4000, 8000, 16000 and 32000 lines, the sizes nothing in the tree merged before. It skips itself with a line where there is neither a diff3 nor a git |
...
| ZP153 | the DIFF column: C where the two sides' content differs, M where their attributes differ, CM where both, "-" where neither or a side is absent; per zo_attrs_equal's rule, which is mode, uid, gid, flags, xattrs, ACL and default ACL | covered: check_picker.c, DIFF values for text with same content, a file with different modes, and a row with one side absent |
| ZP154 | the metadata three-way: from equal to base yields onto's value; onto equal to base yields from's; from equal to onto is that; else a conflict. f picks from, o picks onto, - unpicks, n/p move among conflicts | covered: check_picker.c, a mode conflict resolved with f and o, and an unpick with - |
| ZP155 | w on screen 2 refuses while metadata has unpicked conflicts, naming the view | covered: check_picker.c, the refusal message containing "metadata" |
| ZP156 | w writes the content merge then sets the resolved attributes: chown, xattrs, ACL, mode, flags. This was the apply's order (chown, chmod, xattrs, ACL, flags) until ZP188 moved the mode after the ACL | covered: check_picker.c, a mode change resolved to from's value, an xattr set and one removed, verified after the write |
| ZP157 | a directory whose two sides have the same content and different metadata opens on the metadata view alone, with a content view showing "a directory has no content view" | planned: box, by hand, hand-sessions.md; the model opens the merge on a directory with DIFF_M |
| ZP158 | the c and m keys switch between the content and metadata views; a on screen 2 is conflicts only | covered: check_picker.c on a pty, the key script uses a for conflicts only (was c before this issue) |
| ZP159 | the metadata view on screen 2 on a pty: attribute names (mode, owner) and values (the mode's octal bits) are visible in the drawn output, and "metadata" appears in the title or bar | covered: check_picker.c on a pty, a text file with different modes on from and onto, m pressed, mode and 0640/0600 in the output |
| ZP160 | c from the metadata view switches to the content view, and a folds conflicts only | covered: check_picker.c on a pty, the key sequence m, c, m, a, q, q; the content view's RESULT label appears after c |
| ZP161 | the bar text of each view names the other view's state ("m metadata: N unpicked" or "c content: same" etc.) | covered: check_picker.c on a pty, the word "metadata" in the output of the content view's bar |
| ZP162 | a metadata pick, q, d: the pick is dropped, the result's mode is unchanged (Esc/q on screen 2 asks when metadata picks are made and d drops them, same as the content question of ZP180) | covered: check_picker.c on a pty, m, f, q, d, q; the result's mode read back and unchanged |
| ZP163 | a metadata pick, q, w (or m, f, w with nothing else unpicked): the result's mode is from's, because f picked from's mode in the metadata three-way and w wrote the resolved attributes; the exit status is 1 and the document reads keep | covered: check_picker.c on a pty, m, f, w, q; the result's mode is 0640, WEXITSTATUS is 1, and the resolution carries "keep" |
| ZP164 | w on a CM row (content and metadata both differ) with only the content picked: the write is refused naming metadata, and the result's bytes are unchanged; once metadata is also picked, w lands both the merged bytes and the resolved attributes | planned: box, by hand, hand-sessions.md; the reorder of compute-then-write (review fix M9) is the model change that prevents writing before checking metadata |
| ZP165 | the flags row prints through fflagstostr where the platform has it, "-" for 0, hex only where fflagstostr is unavailable | covered: check_picker.c, zr_flags_to_text on UF_HIDDEN (the mac) returns a name and not hex, and 0 returns "-" |
| ZP166 | the ACL row shows the entries with numeric ids, one line per entry (in ZP192's short form on FreeBSD), with the cell as tall as the longest of its three columns | planned: box, by hand, hand-sessions.md; the mac's extended ACL text is verified by the drawing code on macOS with a real ACE |
| ZP167 | the owner cell shows the uid and the name in parentheses where getpwuid knows it; the number is the value compared | covered: check_picker.c on a pty, the "(" character in the drawn output for the running user; and check_picker.c, getpwuid of the running uid returns a name |
| ZP180 | Esc or q on screen 2 with picks that no write has taken asks once in the key bar: w writes, and refuses as w does while a conflict is unpicked, d drops the picks and goes back to the list, any other key cancels and the merge stays open; with no pick made the screen closes at once (the author, 2026-09-18, hand session 6). The number leaves a gap after ZP152 because the picker-metadata rows were being numbered from ZP153 in a worktree at the time | covered: check_picker.c on a pty: Enter, f, Esc, d leaves the object and the document as they were; Enter, f, q, w writes the merge and closes the screen |
| ZP181 | the metadata merge picked by hand over FreeBSD's own attributes: one file whose content is the same and whose mode, file flags and one user-namespace xattr conflict, with a second user xattr changed on from alone. f on the mode, o on the flags, o on the conflicted xattr, w: the result carries from's mode, onto's flags, onto's value for the conflicted xattr and from's value for the one-sided one, and the document reads keep. Each value is read back with lstat, st_flags and extattr_get_file, not off the screen | covered: check_picker.c on a pty, FreeBSD only (make check-freebsd), test_pty_meta_fbsd. The system namespace is root's and is left to hand session 2 of box-trip-2026-09-16.md |
| ZP182 | the NFSv4 ACL row picked by hand: a directory whose two sides each added a different allow entry (user 1001 on from, user 1002 on onto) opens on the metadata view alone; f on the acl row and w leave the result's ACL with from's entry and without onto's, read back with acl_get_file and acl_to_text_np with numeric ids | covered: check_picker.c on a pty, FreeBSD only, test_pty_meta_fbsd; skipped with a line where the test directory's file system has no NFSv4 ACLs (pathconf _PC_ACL_NFS4) |
| ZP183 | the metadata merge through a real rebase to done on a pool: conflicts on mode, owner, flags, a user xattr, a system xattr and the ACL, some picked f and some o, the picker's w, the run to done, and every value read back on the placed clone. The final check does not compare names answered keep, so the reads are the only check that the merged attributes survived applying2 and done | planned: box, by hand, hand session 2 of box-trip-2026-09-16.md |
| ZP184 | where no tree has any xattr, the metadata view shows one xattrs row reading "-" in every column, as the acl row does, so an empty set reads differently from one never read; it resolves on every side and is never a conflict, and the write skips it (the author, 2026-09-23, box session 6: "we can't see xattrs here, but should") | covered: check_picker.c on a pty, test_pty_meta_view, the word "xattrs" in the drawn metadata view of a row whose trees carry none |
| ZP185 | the other view's state in the bar says "c content: merged" where the two sides' bytes differ and merge with no conflict, and "same" only where they do not differ; before this a clean merge of different bytes read "same" | covered: check_picker.c on a pty, test_pty_meta_view, a CM row whose edits are two lines apart: "c content: merged" drawn and "c content: same" not |
| ZP186 | the ATTR column is as wide as the longest row name, between ten cells and a quarter of the screen, and the value columns start after it, so an xattr name longer than ten is drawn whole (session 6's user.backup.exclude was cut at ten) | covered: check_picker.c on a pty, test_pty_meta_view, a thirteen-byte xattr name found whole in the output at 120 columns |
| ZP187 | the metadata view draws its side rules on every line down to the key bar's rule, including the lines below the last attribute row, as the content view does (the author, 2026-09-23, box session 6, "missing side border glyphs where there's no content") | covered by hand: on the mac, the view rendered through a terminal emulator (pyte) before and after the fix, the rules absent below the acl row before and present after; box, by eye, hand session 6 again. No pty test: the harness reads bytes and has no cell grid to ask which column a glyph landed in |
| ZP188 | the picker sets the mode after the ACL, since the metadata view can take the two from different sides and an NFSv4 ACL carries a mode of its own; where the dataset's aclmode will not let the picked mode stand beside the picked ACL, the ACL wins and the key bar says so: a chmod that replaced the ACL with a trivial one (aclmode=discard, the default) is undone by setting the ACL again, a chmod refused beside it (restricted) is left refused, and either way the line names the mode the object has and the one picked. Where the picked mode stands (passthrough, groupmask, or no ACL) nothing is said. The apply's order is unchanged: it takes both from one side (the author, 2026-09-23, on OpenZFS's zfs_acl_chmod_setattr) | covered: check_picker.c on a pty, FreeBSD only, test_pty_meta_fbsd: a directory whose mode comes from from (0750) and whose NFSv4 ACL comes from onto (a user 1002 entry), nothing to answer, w; the entry is on the result whatever TMPDIR's aclmode is, and either the mode reads 0750 and no warning was drawn or it reads otherwise and the warning was. Which branch ran is printed. The restricted branch is by hand, on a dataset set aclmode=restricted |
| ZP189 | the metadata view's row head is laid out as screen 2's content cells are (ZP118): the marker, a space, the cursor's cell, a space, then the name, and the ATTR header over the name (the author, 2026-09-23, box session 6: "[!/f/o] space [highlight block] space [content]") | covered by hand: on the mac, rendered through a terminal emulator (pyte), the marker in column 1, the cursor's cell in column 3 in the band's colors, the name in column 5; box, by eye. No pty test, for ZP187's reason |
| ZP190 | the metadata view colors its markers as the content view colors its gutter: ! and an unanswered row's name red, f or o and the row's name in the answering side's own color once picked, + green (the author, 2026-09-23: "when picking f/o, the letter stays red in this view") | covered by hand: on the mac, the per-cell colors read off the pyte render after o on the mode row and f on an xattr row: o and its name in onto's color, f and its name in from's, + green; box, by eye |
| ZP191 | an ACL cell draws each entry from its first character, never flush right, and the value columns take a quarter of the width left after ATTR with no upper cap, each drawing one cell narrower than it is wide so a cut cell never runs into the next column (the author, 2026-09-23: "the acls are right aligned", and the next screenshot, where a cut entry ran into its neighbour) | covered by hand: the gap on the mac in a pyte render; the alignment, box by eye |
| ZP192 | an NFSv4 ACL is drawn in the short form setfacl takes, built from the entries (acl_get_entry and the per-entry getters, never by editing acl_to_text_np's string): the tag, the letters of the permissions set in libc's own order (lib/libc/posix1e/acl_support_nfs4.c), the flags only where there are any, and the type, as "user:80:rwxp:allow" for acl_to_text_np's "user:80:rwxp----------:-------:allow". A POSIX.1e ACL, already short, is drawn as acl_to_text_np writes it. Display only (the author, 2026-09-23: "the near uninterrupted stream of hyphens is awful to look at"; a left-padded aligned form is for later) | covered: check_picker.c on a pty, FreeBSD only, test_pty_meta_fbsd's ZP182 case at 160 columns: "user:1001:rwx:allow" drawn, and "rwx-" nowhere |
| ZP193 | the built-in picker's two refresh hooks share one core, run in the picker's child: onto and from walked at the .zfs/snapshot directories the gate walked and handed the picker, the result at the private mount, the oracle over onto, from and the result, the manifest held, the resolution as it is on disk now, the missing conflict lines put back, a drift line per moved name, one write where anything changed, and nothing printed while curses has the terminal. No libzfs in the child: the launcher closed /dev/zfs with every other descriptor (L6), and the resume hook's mounted/mountpoint lookup met EBADF on the box (the author, 2026-09-23, "refresh failed: zrm/lo: cannot open 'zrm/lo': Bad file descriptor"). The same review of the code found the fresh hook verifying against an oracle over base, from and onto and a result walk released before the fork, and the resume hook classifying against the resolution as read before the fork, which an r after an s would have written over the saved answers | covered: by hand on the box, 2026-09-23, rewritten hand session 1 of box-trip-2026-09-16.md on tool fdab53d; before that planned: box, by hand, hand session 1 steps 4 and 10 of box-trip-2026-09-16.md, as ZP151. No harness on either machine drives the built-in picker's r over real trees: the -i cases use a script as the editor, and the mac has no pool |
| ZP194 | the detail line's link clause is never the part cut: it goes after the sizes and before the group text, which gives way first; where the whole clause does not fit there, "held under N names" does; where not even that fits, the short form opens the line and the sizes are cut (the author, 2026-09-23, box session 1: 94 columns for the whole clause with three tiny files, and more with real sizes, which are bytes) | covered by hand: on the mac, rendered through a terminal emulator at 80, 94, 100 and 130 columns over a linked pair of 14 bytes and one of 11.9 MB, each branch seen; check_picker.c's ZP140 pty case still finds "under 2 names" at 80 columns |
