...
| ZP138 | q on the list with answers that no write has saved: no prompt and no question, one line on stderr naming how many answers were not saved, printed after the terminal is the person's again, and the exit status q has always had; with nothing unsaved, q says nothing (the author, 2026-09-15, on M9: zr_pk_dirty is the count's source and was never called) | covered: check_picker.c on a pty for both, the line asserted to lie past the last escape byte the way ZP68 does it, and over the model for the count itself |
