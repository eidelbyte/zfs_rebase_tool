...
| `--abort` | `-A` | from applying2, the result rolled back to the snapshot the gate's hand-off took and the rebase left standing at the conflicts gate; anywhere else, holds released, tool-made snapshots destroyed, the clone destroyed or the dataset rolled back, the run directory and the documents in it removed (a `-o` pair stays) |
...
does one of two things. From applying2 it rolls the result back to
the snapshot the tool took of it at the hand-off into that stage and
stops there, leaving the rebase standing at the conflicts gate with
its record, its holds, its documents and its run directory: everything
answered and hand merged at that gate survives, --continue takes the
rebase on from it, and a second --abort, which now finds a rebase at
conflicts, takes the whole thing away. Anywhere else -- and from
applying2 where that snapshot is gone -- it
