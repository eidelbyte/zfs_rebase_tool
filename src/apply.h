 ...
 * and never follows a symbolic link, except the calls that have no
 * descriptor-relative form -- the extended attributes, the ACL and
 * the file flags everywhere, and off FreeBSD the bind(2) that makes
 * a socket, each handed the root's own path with the action's path
 * appended. What keeps those inside the tree is not a resolved
 * descriptor per component but the 0700 chain down to the run
 * directory and the mount policy that puts the result at its mnt,
 * root's alone (R17 of the code review, filed in
 * sprints/future-features.md). FreeBSD's own bindat(2) takes the
 * parent descriptor, so on the target the socket is not among them.
 * Conflict marks do nothing.
 *
